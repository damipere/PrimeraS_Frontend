...
/*se asiganan los colores para los tipos y clases*/
const typeColors = {
    electric: '#FFEA70',
    normal: '#B09398',
    fire: '#FF675C',
...
/*se realiza la busqueda de el pokemon seleccionado*/
const fetchPokemon = () => {
    ...
    const url = `https://pokeapi.co/api/v2/pokemon/${pokeName}`;
        fetch(url).then((res) => {
            if (res.status != "200") {
                pokeImage("img/pokeball.png","Pokemon not found","Pokemon no encontrado");
                ancho.style.maxWidth="200px";
                oculta.style.display="none";
                oculta2.style.display="none";
                tipopokemon.style.display="none";
                /*oculata elementos en dado caso que no se encuentre el pokemon seleccionado*/
            }
            else {
                return res.json();
            }
...
/*remplaza la imagen de el html por la del pokemon seleccionado*/
const pokeImage = (url,name,id) => {
    pokePhoto.src = url;
    /*remplaza el numero y el nombre del pokemon*/
    pokeid.innerHTML="No. "+id;
    pokenamebus.innerHTML=name;
}

/*colorea las clases y tipo de pokemon*/
const pokeTypes = (types) =>{
    /*limpia el elemento de tipo de pokemon*/
    tipopokemon.innerHTML=""
    /*realiza un recorrido para los tipos de el pokemon*/
    types.forEach(element => {
        /*crea un div para cada tipo*/
        const tipoelemento = document.createElement("div");
        /*colorea el texto*/
        tipoelemento.style.cssText='color:'+typeColors[element.type.name]+'; font-weight: bold;';
        /*asigna el texto*/
        tipoelemento.textContent = element.type.name;
        /*lo agrega al padre para que sea visible*/
        tipopokemon.appendChild(tipoelemento);
    });
    /*le cambia el color del fondo de la imagen*/
    pokeImg.style.background =`radial-gradient(#FFFFFF 33%, ${typeColors[types[0].type.name]} 33%)`;
    pokeImg.style.backgroundSize = '10px 10px';
}
/*Funcion estadisitacas pokemon*/
const pokeEstadisticas=(stats) =>{
    let a=0,b=0;
    /*for para recorrer las estadisticas*/
    stats.forEach(element=>{
        /*crea las varias de los elementos de html para el texto y los numeros de las estadisticas*/
        const hp = document.getElementById("stats"+a); /*numero*/
        const text = document.getElementById("text"+a); /*texto*/
        b=(element.base_stat*100)/255 /*operacion para sacar el porcetanje tomando en cuenta que segun internet el pokemon con mas fuerza es de 300*/
        hp.style.width=b+"%"; /*se asigna el porcentaje a cada barra de progreso*/
        text.textContent=element.stat.name+":  "+element.base_stat; /* se asigna el texto y numero de estadisitica*/ 
        a++; /*aumenta el contador para el recorrido de html*/
    });
}
/*funcion para autoacompletar al buscar un pokemon*/
  $(function() {
    ...
    $( "#pokeName" ).autocomplete({
      ...
      select : function(event, ui){ /*funcion para detectar cuando seleccionan algun pokemon*/
          /*se asigna el pokemon seleccionado para poder realizar la busqueda*/
        $("#pokeName").val(ui.item.value);
        /*llamado de funcion para busqueda de pokemon*/
        fetchPokemon();
    }
    });
  });
  /*funcion para detectar cuando hacen enter en el input de busqueda
  $("#pokeName").on('keyup', function (event) {
    if (event.keyCode === 13) {
        fetchPokemon(); /*llama la funcion de busqueda de pokemon
    }
});*/
